...
class EvMasterInfo extends CommonInfo {
    // 확정형
    var $sell_price       = null;
    ...
    function init() {
        ...
        $dao->selectCateSizeHtml($conn, $param, $price_info_arr, $pos_flag);

        $template->reg($prefix . "size_mpcode", $price_info_arr["stan_mpcode"]);
        ...
        if ($mono_dvs === '0') {
            $sell_price  = doubleval($dao->selectPrdtPlyPrice($conn, $param));
