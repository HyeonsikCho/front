<?php
    define("HOME_DIR", $_SERVER["DOCUMENT_ROOT"] . "/webpay_vacnt");
    ...
    $return_url  = $_POST["return_url"];         //결과 페이지
    ...
    $user_define1     = $_POST["EP_user_define1"];     // [선택]가맹점필드1
    $user_define2     = $_POST["EP_user_define2"];     // [선택]가맹점필드2
    $user_define3     = $_POST["EP_user_define3"];     // [선택]가맹점필드3
    $user_define4     = $_POST["EP_user_define4"];     // [선택]가맹점필드4
    $user_define5     = $_POST["EP_user_define5"];     // [선택]가맹점필드5
    $user_define6     = $_POST["EP_user_define6"];     // [선택]가맹점필드6
 
    /* -------------------------------------------------------------------------- */
    /* ::: 기타정보 설정                                                          */
    /* -------------------------------------------------------------------------- */
    $tot_amt          = $_POST["EP_tot_amt"];          // [필수]결제 총 금액
    $currency         = $_POST["EP_currency"];         // [필수]통화코드
    $escrow_yn        = $_POST["EP_escrow_yn"];        // [필수]에스크로여부
    $complex_yn       = $_POST["EP_complex_yn"];       // [필수]복합결제여부
    $tax_flg          = $_POST["EP_tax_flg"];          // [필수]과세구분 플래그(TG01:복합과세 승인거래)
    $com_tax_amt      = $_POST["EP_com_tax_amt"];      // [필수]과세 승인 금액(복합과세 거래 시 필수)
    $com_free_amt     = $_POST["EP_com_free_amt"];     // [필수]비과세 승인 금액(복합과세 거래 시 필수)
    $com_vat_amt      = $_POST["EP_com_vat_amt"];      // [필수]부가세 금액(복합과세 거래 시 필수)
    ...
    if( $TRAN_CD_NOR_PAYMENT == $tr_cd ) {
        ...
        $easyPay->set_easypay_deli_us( $comm_data, "currency"      , $currency      );
        ...
        $easyPay->set_easypay_deli_us( $comm_data, "tax_flg"       , $tax_flg       );
        $easyPay->set_easypay_deli_us( $comm_data, "com_tax_amt"   , $com_tax_amt   );
        $easyPay->set_easypay_deli_us( $comm_data, "com_free_amt"  , $com_free_amt  );
        $easyPay->set_easypay_deli_us( $comm_data, "com_vat_amt"   , $com_vat_amt   );
        ...
        if( $cash_yn == "1" ) {
            ...
            $easyPay->set_easypay_deli_us( $acnt_data, "cash_issue_type" , $cash_issue_type  );
            $easyPay->set_easypay_deli_us( $acnt_data, "cash_auth_type"  , $cash_auth_type   );
            $easyPay->set_easypay_deli_us( $acnt_data, "cash_auth_value" , $cash_auth_value  );
        }
...
<form name="frm" method="post" action="./return.php">
    <input type="hidden" name="return_url"      value="<?=$return_url?>">      <!-- 결과 페이지 -->
