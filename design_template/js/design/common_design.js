$(document).ready(function () {
    ...
    /*
    //마우스오버
    $('header.top ._order').bind('mouseover', function () {
        $('header.top ._order .wrap').stop();
        $('header.top ._order .wrap').slideDown(150);
        $('header.top ._order button._closed').closest('._order').addClass('_on');
    });

    $('header.top ._order').bind('mouseout', function () {
        $('header.top ._order .wrap').stop();
        $('header.top ._order .wrap').slideUp(150);
        $('header.top ._order button._opened').closest('._order').removeClass('_on');
    });
    */
...
function layerPopup (code, html) {
    var layerPopupHTML = '<div class="modalMask ' + code +' _num' + layerPopNum + '" style="z-index:100;"><div class="loading">불러오는 중입니다.</div><div class="layerPopupWrap"><section class="layerPopup"></section></div></div>';
