$(document).ready(function () {
    ...
    preview.paper = preview.container.find('.paper');
    ...
    preview.btns = preview.container.find('.btns');
    preview.lines = preview.container.find('.line')
    preview.cuttingline = preview.lines.filter('.cutting');
    preview.workingline = preview.lines.filter('.working');
    ...
    $('figure.picture .thumb').on('click', function () {
        ...
        $("#pic_view").attr("data-magnify-src", target2);
...
var initAfter = function() {
    ...
    //preview - 귀도리
    $('.after li._rounding input[type=checkbox]').on('click', function () {
        if ($(this).prop('checked')) {
            //귀도리 선택
            $('.after ._rounding select._num').next().change();
            $('.after ._rounding select._num').change(preview.rounding);
        } else {
            //귀도리 해제
            $('.after .option._rounding input[type=checkbox]').prop('checked', false);
            preview.rounding();
        }
    });
...
var preview = {
    ...
    lines : null,
    paperSize : function () {
        if (Number(preview.workingSizeW.val()) >= Number(preview.workingSizeH.val())) {
            preview.PPM = 350 / preview.workingSizeW.val();
        } else {
            preview.PPM = 350 / preview.workingSizeH.val();
        }
        
        preview.workingline.width(Number(preview.workingSizeW.val()) * preview.PPM);
        preview.workingline.height(Number(preview.workingSizeH.val()) * preview.PPM);
        preview.cuttingline.width(Number(preview.cuttingSizeW.val()) * preview.PPM);
        preview.cuttingline.height(Number(preview.cuttingSizeH.val()) * preview.PPM);
        preview.paper.width(Number(preview.cuttingSizeW.val()) * preview.PPM);
        preview.paper.height(Number(preview.cuttingSizeH.val()) * preview.PPM);
        preview.rounding();
    },
    borderRadius : 0,
    borderRadiusStyle : '',
    rounding : function () {
        ...
        $('.after .option._rounding input[type=checkbox]').each(function () {
            if ($(this).prop('checked')) {
        ...
        preview.paper.css('border-radius', preview.borderRadiusStyle);
    }
}
