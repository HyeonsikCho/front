$(document).ready(function () {
    ...
    //!! 일반명함 후공정 부분 슬라이드 바인딩
    $('.cut_after ._closed').on('click', function () {
        $('.cut_after_list').stop().slideDown(300);
    });
    $('.cut_after ._opened').on('click', function () {
        $('.cut_after_list').stop().slideUp(300);
    });
    ...
    */
    ...
    //!! 일반명함 사이즈 부분 초기화
    size("cut");
    $('.cut_size select').on('change', function () {
        size("cut");
    });
    $('.cut_size ._cuttingSize input[type=text]').on('blur', function () {
        size("cut");
        calcManuPosNum.exec("cut");
    });
    ...
    */
    ...
    // 자리수 계산 함수 호출
    calcManuPosNum.defWid  = parseFloat($("#cut_size").attr("def_cut_wid"));
    calcManuPosNum.defVert = parseFloat($("#cut_size").attr("def_cut_vert"));
});
...
function afterOverview () {
    ...
    var afterAddHtml = "";
    $('.after .option._on dl').each(function () {
