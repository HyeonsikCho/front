...
$(document).ready(function() {
    ...
    monoYn   = $("#etc_mem_mono_yn").val();
    tmptDvs  = $("#etc_mem_tmpt_dvs").val();
    prdtDvs  = $("#prdt_dvs").val();
    sortcode = $("#etc_mem_cate_sortcode").val();
    cateName = $("#etc_mem_cate_sortcode").find("option:selected").text();
});
...
var changeData = function() {
    ...
    data.print_purp         = $("#etc_mem_print_purp").val();
    data.page_info          = "2";

    if ($("#size_dvs").val() === "manu") {
        data.stan_mpcode   =
            $("#etc_mem_size").attr("def_val");
        data.def_stan_name =
            $("#etc_mem_size > option[value='" + data.stan_mpcode + "']").html();
        data.manu_pos_num  =
            $("#manu_pos_num").val();
        data.amt_unit      =
            $("#amt").attr("amt_unit");
...
var calcPrice = function() {
    ...
    var posNum = 1;
    if ($("#size_dvs").val() === "manu") {
        posNum = parseFloat($("#etc_mem_manu_pos_num").val());
    }
    // 건수
 ...
 */
var getEtcMemAfterPriceCommon = function(aft, name, selector) {
 ...
 * @param aft = 후공정 구분값
 */
var getEtcMemDotlinePrice = function(aft, name) {
    var selector = "input[name='etc_mem_" +
                   aft + '_' +
                   $("#etc_mem_" + aft + "_cnt").val() +
                   "_val']:checked";
    ...
    getEtcMemAfterPriceCommon(aft, name, selector);
};

/**
 * @brief 명함 오시 가격 계산 함수, 미싱 가격하고 동일
 */
var getEtcMemImpressionPrice = getEtcMemDotlinePrice;
...
var getCoatingPlySheetPrice = function(aft) {
    getEtcMemAfterPriceCommon(aft, "코팅", null);
};
...
var getRoundingPlySheetPrice = function(aft) {
    getEtcMemAfterPriceCommon(aft, "귀도리", null);
};
...
var getEmbossingPlySheetPrice = function(aft) {
    getEtcMemAfterPriceCommon(aft, "엠보싱", null);
};
...
var getPunchingPlySheetPrice = function(aft) {
    getEtcMemAfterPriceCommon(aft, "타공", null);
};
...
var getImpressionPlySheetPrice = function(aft) {
    getEtcMemImpressionPrice(aft, "오시");
};
...
var setSubmitParam = function() {
    ...
    var sizeName  = $("#etc_mem_size").find("option:selected").text();
    ...
    $frm.find("input[name='etc_mem_cate_name']").val(cateName);
    $frm.find("input[name='etc_mem_amt_unit']").val(amtUnit);
    $frm.find("input[name='etc_mem_size_name']").val(sizeName);

    $frm.find("input[name='etc_mem_prdt_price']").val(prdtPrice);
    $frm.find("input[name='etc_mem_basic_price']").val(basicPrice);
    $frm.find("input[name='etc_mem_grade_sale_rate']").val(gradeSaleRate);
    $frm.find("input[name='etc_mem_after_price']").val(afterPrice);
    $frm.find("input[name='etc_mem_opt_price']").val(optPrice);
};
