...
$(document).ready(function() {
    ...
    tmptDvs  = $("#st_tms_tmpt_dvs").val();
    prdtDvs  = $("#prdt_dvs").val();
    sortcode = $("#st_tms_cate_sortcode").val();
    cateName = $("#st_tms_cate_sortcode").find("option:selected").text();
});
...
var changeData = function() {
    ...
    data.print_purp         = $("#st_tms_print_purp").val();
    data.page_info          = "2";
 ...
 */
var getTomsonAfterPriceCommon = function(aft, name, selector) {
...
var getCoatingPlySheetPrice = function(aft) {
    getTomsonAfterPriceCommon(aft, "코팅", null);
};
...
var setSubmitParam = function() {
    ...
    var sizeName  = $("#st_tms_size").find("option:selected").text();
    ...
    $frm.find("input[name='st_tms_cate_name']").val(cateName);
    $frm.find("input[name='st_tms_amt_unit']").val(amtUnit);
    $frm.find("input[name='st_tms_paper_name']").val(paperName);
    $frm.find("input[name='st_tms_bef_tmpt_name']").val(tmptName);
    $frm.find("input[name='st_tms_size_name']").val(sizeName);

    $frm.find("input[name='st_tms_prdt_price']").val(prdtPrice);
    $frm.find("input[name='st_tms_basic_price']").val(basicPrice);
    $frm.find("input[name='st_tms_grade_sale_rate']").val(gradeSaleRate);
    $frm.find("input[name='st_tms_after_price']").val(afterPrice);
    $frm.find("input[name='st_tms_opt_price']").val(optPrice);
};
...
var makeEstiPopInfo = {
    ...
    "exec" : function(dvs) {
        ...
        var size    = $("#size_typ > option:selected").text() + '/' +
                      $(prefix + "size > option:selected").text();
