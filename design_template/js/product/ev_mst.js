...
$(document).ready(function() {
    ...
    monoYn     = $("#ev_mst_mono_yn").val();
    tmptDvs    = $("#ev_mst_tmpt_dvs").val();
    prdtDvs    = $("#prdt_dvs").val();
    sortcode   = $("#ev_mst_cate_sortcode").val();
    cateName   = $("#ev_mst_cate_mid").find("option:selected").text();
    stanMpcode = $("#ev_mst_size").val();
});
...
var changeData = function() {
    ...
    data.print_purp         = $("#ev_mst_print_purp").val();
    data.page_info          = "2";
 ...
 */
var changeEvTmpt = function(val) {
    $("input[name='ev_mst_tmpt_chk']").prop("checked", false);
    $("input[name='ev_mst_tmpt_chk']").prop("disabled", false);
...
var chkTmptLim = function() {
    ...
    if (tmpt === "1도") {
        if ($("input[name='ev_mst_tmpt_chk']:checked").length > 0) {
            $("input[name='ev_mst_tmpt_chk']").prop("disabled", true);
        } else {
            $("input[name='ev_mst_tmpt_chk']").prop("disabled", false);
        }
    } else {
        if ($("input[name='ev_mst_tmpt_chk']:checked").length > 1) {
            $("input[name='ev_mst_tmpt_chk']").prop("disabled", true);
        } else {
            $("input[name='ev_mst_tmpt_chk']").prop("disabled", false);
        }
    ...
    $("input[name='ev_mst_tmpt_chk']:checked").prop("disabled", false);
};
...
var makeTmptName = function() {
    var ret = $("#print_tmpt").find("option:selected").text() + " - ";

    var bef = "전면 : ";
    $("#bef").find("input[name='ev_mst_tmpt_chk']:checked").each(function() {
        bef += $(this).val();
        bef += ' ';
    });
    ...
    var aft = "후면 : ";
    $("#aft").find("input[name='ev_mst_tmpt_chk']:checked").each(function() {
        aft += $(this).val();
        aft += ' ';
    });
...
var setSubmitParam = function() {
    ...
    if ($("input[name='ev_mst_tmpt_chk']:checked").length === 0) {
        return alertReturnFalse("인쇄색상을 선택해주세요.");
    }

    if ($(prefix + "size").val() === "2도") {
        if ($("input[name='ev_mst_tmpt_chk']:checked").length === 1) {
            return alertReturnFalse("인쇄색상을 선택해주세요.");
        }
    }
    ...
    var sizeName  = $(prefix + "cate_sortcode").find("option:selected").text();
    ...
    $("#tmpt_name").val(tmptName);

    $frm = $("#frm");

    $frm.find("input[name='ev_mst_cate_name']").val(cateName);
    $frm.find("input[name='ev_mst_amt_unit']").val(amtUnit);
    $frm.find("input[name='ev_mst_paper_name']").val(paperName);

    $frm.find("input[name='ev_mst_prdt_price']").val(prdtPrice);
    $frm.find("input[name='ev_mst_basic_price']").val(basicPrice);
    $frm.find("input[name='ev_mst_grade_sale_rate']").val(gradeSaleRate);
    $frm.find("input[name='ev_mst_after_price']").val(afterPrice);
    $frm.find("input[name='ev_mst_opt_price']").val(optPrice);
};
...
var makeEstiPopInfo = {
    ...
    "exec" : function(dvs) {
        ...
        var cateSize = $(prefix + "cate_sortcode > option:selected").text();
