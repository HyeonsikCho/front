<?php
    ...
    define("HOME_DIR", $_SERVER["DOCUMENT_ROOT"] . "/webpay_card_prepay");
    include(HOME_DIR . "/web/easypay_client.php");

    if ($is_login === false) {
        echo "<script>alert(\"로그인 상태가 아닙니다.\"); location.replace(\"/member/login.html\");</script>";
        exit;
    }
    ...
    if ( $res_cd == "0000" ) {
        ...
        if ($r_pay_type === "11") {
            ...
            $exist_prepay = $dao->selectPrepayPrice($conn, $member_seqno);
            $exist_prepay = $exist_prepay->fields["prepay_price"];

            $prepay_bal = $exist_prepay + intval($r_amount);
            ...
            $param["depo_way"]      = "카드";
            $param["exist_prepay"]  = $exist_prepay;
            $param["prepay_bal"]    = $prepay_bal;
            $param["state"]         = "결제완료";
            $param["deal_num"]      = $r_order_no;
            $param["prepay_use_yn"] = '-';
            ...
            if ($proc_ret === false) {
                $conn->FailTrans();
                $conn->RollbackTrans();
                echo "<script>alert('회원결제내역 입력에 실패했습니다.'); closePopup(popupMask);</script>";
                exit;
            }

            // 회원 선입금액 테이블 값 수정
            unset($param);
            $param["prepay_price"] = $prepay_bal;
            $param["member_seqno"] = $member_seqno;
            ...
            if ($proc_ret === false) {
                $conn->FailTrans();
                $conn->RollbackTrans();
                echo "<script>alert('선입금액 수정에 실패했습니다.'); closePopup(popupMask);</script>";
                exit;
            }
            ...
            if ($rs->EOF) {
                unset($param);
                $param["public_object_price"]   = '0';
                $param["unissued_object_price"] = $r_amount;
                $param["member_seqno"]          = $member_seqno;

                $proc_ret = $dao->insertPublicObjectPrice($conn, $param);

            } else {
                $unissued_price  = intval($rs->fields["unissued_object_price"]);
                $unissued_price += $unissued_price + intval($r_amount);
            ...
            if ($proc_ret === false) {
                $conn->FailTrans();
                $conn->RollbackTrans();
                echo "<script>alert('발행대상금액 수정에 실패했습니다.'); closePopup(popupMask);</script>";
                exit;
            }

            $conn->CompleteTrans();
            // 회원 선입금액 세션 값 수정
            $_SESSION["prepay_price"] = $prepay_bal;
        }
...
<body onload="window.parent.goCharge();">
    ...
    <input type="hidden" id="prepay_bal"       value="<?=$prepay_bal?>">       <!-- 환불예정일시 //-->
</body>
</html>
